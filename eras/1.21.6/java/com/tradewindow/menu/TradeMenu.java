 ...
 * Mojang's official mappings, where it is {@code clicked(int, int, ClickType, Player)}.)
 *
...
public class TradeMenu extends AbstractContainerMenu {
	 ...
	 */
	private void pressHeader(int slotId, ClickType clickType, Player player) {
		if (clickType != ClickType.PICKUP || !(player instanceof ServerPlayer server)) {
			return;
		}
